...
def _synth() -> int:
    ...
    print("\nScenario 4: f(n) = fibonacci(n), true zero-indexed F(0)=0/F(1)=1, from 7 I/O")
    print("examples (asymmetric recursion - needs f(n-1) AND f(n-2), base case is the")
    print("parameter itself (base_kind='param'), not a fixed constant)")
    fib = [0, 1, 1, 2, 3, 5, 8]
    ...
    print(
        "(Asymmetric recursion is NOT reliable the way 2**n is: some seeds fail to "
        "verify, and one otherwise-good seed 'verifies' against training examples "
        "with a degenerate expression that doesn't generalize. Adding more training "
        "examples fixes some seeds but breaks others - a genuine whack-a-mole, not a "
